...
**IMPORTANT: Neuron Wallet support to claim vesting/locked tokens. Please note that this feature is currently experimental.Development is still happening so there might be changes.**
...
When the lock time is reached，click `Claim` and input the wallet's password then you can claim the asset.
