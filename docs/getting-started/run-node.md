...
Open the `ckb.toml` file with a text editor, and then add these parameters to the end of the file (replace `ckb cli blake160 <compressed-pubkey>` with the `lock_arg` you got from [wallet creation](./wallet#create-wallet) as a 0x-prefixed hex string; the `code_hash` here should NOT be changed):
```toml
[block_assembler]
